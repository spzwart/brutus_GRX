...
// 1PN pair
void Acceleration::Acceleration_PN1_pair(array<mpreal, 3> &da, array<mpreal, 3> &dr, array<mpreal, 3> &dv, array<mpreal, 3> &vj) {
    mpreal PairInProduct, Pairn;
    
    for(int k=0; k<3; k++) {
        PairInProduct += dr[k]*("4"*dv[k]+vj[k]);
    }
    ...
    for(int k=0; k<3; k++) {
        da[k] += c2*mj*apreij*PairInProduct*dv[k] + Pairn*dr[k];
    }
}
...
void Acceleration::calcAcceleration() {
    ...
    for (vector<Star>::iterator si = s.begin(); si != s.end(); ++si) {
        ...
        for (vector<Star>::iterator sj = s.begin(); sj != s.end(); ++sj) {
            ...
            if (PN1p)   Acceleration_PN1_pair(da, dr, dv, vj);
